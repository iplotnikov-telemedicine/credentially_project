WITH rjpv AS (
        SELECT o.id AS organisation_id,
            ...
            NULLIF(concat_ws(' ', e.first_name, e.last_name), '') AS full_name,
            ...
            case pt.staff_type
                ...
                when 'NON_CLINICAL' THEN 'Non-Clinical'
            END as staff_type,
            c.name AS country,
    ...
    tags AS (
        SELECT 
            jt.job_position_id as jp_id,
            string_agg(t_1.name, ', ' ORDER BY jt.create_timestamp desc) AS tags
    ...
    SELECT 
        ...
        rjpv.professional_registration_type,
        ...
        rjpv.full_name,
        rjpv.full_name || ' (' || rjpv.job_position_role || ')' as full_name_with_role,
        t.tags,
        rjpv.create_timestamp
    FROM rjpv
    LEFT JOIN tags t 
    ON t.jp_id = rjpv.jp_id